...
import { Calendar, Download, FileSpreadsheet, Trash2, Clock, ChevronLeft, ChevronRight, User, CheckCircle2 } from 'lucide-react';
...
export default function App() {
  ...
  const [missionName, setMissionName] = useState("Mission Client Alpha");
  ...
  const handleMouseEnter = (dayIndex: number, slotIndex: number) => {
    ...
    // Pour faire un rectangle parfait, on devrait recalculer tout le rectangle depuis le start
    // Mais pour un emploi du temps, "peindre" au passage est souvent plus intuitif pour corriger
    
    // Amélioration : Peindre le rectangle entre Start et Current
    // On réinitialise la grille temporaire pour visualiser le rectangle ?
    // Simplification pour UX fluide : On modifie juste la case survolée si elle est dans la même colonne
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(11);
    doc.text(`Mission / Objet : ${missionName}`, 15, 35);
    ...
    doc.roundedRect(190, finalY, 90, 25, 3, 3, 'FD');
  ...
  const exportExcel = () => {
     ...
     summaryData.push({ Jour: 'TOTAL', Date: '', Horaires: '', Total: calculateHours() });
